...
class ProductResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()
            ->with(['laboratory:id,name', 'createdBy:id,name,last_name', 'updatedBy:id,name,last_name']);
    }

    public static function getNavigationBadge(): ?string
    {
        return (string) Cache::remember(
            'nav-badge:products:total',
            60,
                            ...
                            ->schema([
                                Grid::make(3)->schema([
                                    ...
                                    Select::make('laboratory_id')
                                        ->label('Laboratorio')
                                        ->options(fn () => Laboratory::query()->orderBy('name')->pluck('name', 'id'))
                                        ->searchable()
                                        ->preload()
                                        ->required()
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('name')
                    ...
                    ->wrap()
                    ->description(fn (Product $record) => $record->brand ? "{$record->brand} • {$record->model}" : ($record->serial_number ?? 'Sin serie'), 'after'),
